...
public final class NativeInetAddress {
    ...
    public static io.netty.channel.unix.NativeInetAddress newInstance(InetAddress addr) {
        byte[] bytes = addr.getAddress();
        if (addr instanceof Inet6Address) {
            return new io.netty.channel.unix.NativeInetAddress(bytes, ((Inet6Address) addr).getScopeId());
        } else {
            // convert to ipv4 mapped ipv6 address;
            return new io.netty.channel.unix.NativeInetAddress(ipv4MappedIpv6Address(bytes));
        }
    }
