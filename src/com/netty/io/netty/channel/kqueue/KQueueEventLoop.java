...
final class KQueueEventLoop extends SingleThreadEventLoop {
    private static final InternalLogger logger = InternalLoggerFactory.getInstance(io.netty.channel.kqueue.KQueueEventLoop.class);
    private static final AtomicIntegerFieldUpdater<io.netty.channel.kqueue.KQueueEventLoop> WAKEN_UP_UPDATER =
            AtomicIntegerFieldUpdater.newUpdater(io.netty.channel.kqueue.KQueueEventLoop.class, "wakenUp");
    ...
    private final Callable<Integer> pendingTasksCallable = new Callable<Integer>() {
        ...
        public Integer call() throws Exception {
            return io.netty.channel.kqueue.KQueueEventLoop.super.pendingTasks();
        }
    };
    ...
    KQueueEventLoop(EventLoopGroup parent, Executor executor, int maxEvents,
        ...
        this.kqueueFd = io.netty.channel.kqueue.Native.newKQueue();
        if (maxEvents == 0) {
            allowGrowing = true;
            maxEvents = 4096;
        } else {
        ...
        changeList = new io.netty.channel.kqueue.KQueueEventArray(maxEvents);
        eventList = new io.netty.channel.kqueue.KQueueEventArray(maxEvents);
        jniChannelPointers = new NativeLongArray(4096);
        int result = io.netty.channel.kqueue.Native.keventAddUserEvent(kqueueFd.intValue(), KQUEUE_WAKE_UP_IDENT);
        if (result < 0) {
            cleanup();
    ...
    void evSet(io.netty.channel.kqueue.AbstractKQueueChannel ch, short filter, short flags, int fflags) {
        changeList.evSet(ch, filter, flags, fflags);
    }

    void remove(io.netty.channel.kqueue.AbstractKQueueChannel ch) throws IOException {
        assert inEventLoop();
        if (ch.jniSelfPtr == 0) {
            return;
        }
    ...
    private void wakeup() {
        io.netty.channel.kqueue.Native.keventTriggerUserEvent(kqueueFd.intValue(), KQUEUE_WAKE_UP_IDENT);
    ...
    private int kqueueWait(int timeoutSec, int timeoutNs) throws IOException {
        ...
        int numEvents = io.netty.channel.kqueue.Native.keventWait(kqueueFd.intValue(), changeList, eventList, timeoutSec, timeoutNs);
        changeList.clear();
        return numEvents;
    }
    ...
    private void processReady(int ready) {
        for (int i = 0; i < ready; ++i) {
            ...
            if (filter == io.netty.channel.kqueue.Native.EVFILT_USER || (flags & io.netty.channel.kqueue.Native.EV_ERROR) != 0) {
                ...
                assert filter != io.netty.channel.kqueue.Native.EVFILT_USER ||
                        (filter == io.netty.channel.kqueue.Native.EVFILT_USER && eventList.fd(i) == KQUEUE_WAKE_UP_IDENT);
                continue;
            }

            io.netty.channel.kqueue.AbstractKQueueChannel channel = eventList.channel(i);
            if (channel == null) {
            ...
            if (filter == io.netty.channel.kqueue.Native.EVFILT_WRITE) {
                unsafe.writeReady();
            } else if (filter == io.netty.channel.kqueue.Native.EVFILT_READ) {
                // Check READ before EOF to ensure all data is read before shutting down the input.
                unsafe.readReady(eventList.data(i));
            } else if (filter == io.netty.channel.kqueue.Native.EVFILT_SOCK && (eventList.fflags(i) & io.netty.channel.kqueue.Native.NOTE_RDHUP) != 0) {
                unsafe.readEOF();
            }
            ...
            if ((flags & io.netty.channel.kqueue.Native.EV_EOF) != 0) {
                unsafe.readEOF();
            }
        }
    }
