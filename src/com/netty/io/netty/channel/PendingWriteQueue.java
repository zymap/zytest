 ...
 * {@linkplain io.netty.channel.Channel#isWritable() writability} of the associated {@link Channel}, so that
 * the pending write operations are also considered to determine the writability.
 */
public final class PendingWriteQueue {
    private static final InternalLogger logger = InternalLoggerFactory.getInstance(io.netty.channel.PendingWriteQueue.class);
    ...
    public PendingWriteQueue(ChannelHandlerContext ctx) {
        tracker = io.netty.channel.PendingBytesTracker.newTracker(ctx.channel());
        this.ctx = ctx;
    }
     ...
     *          if the {@link io.netty.channel.PendingWriteQueue} is empty.
     */
