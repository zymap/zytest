...
public final class EpollServerSocketChannel extends AbstractEpollServerChannel implements ServerSocketChannel {
    ...
    public EpollServerSocketChannel(int fd) {
        ...
        this(new io.netty.channel.epoll.LinuxSocket(fd));
    }

    EpollServerSocketChannel(io.netty.channel.epoll.LinuxSocket fd) {
        super(fd);
        config = new EpollServerSocketChannelConfig(this);
    }

    EpollServerSocketChannel(io.netty.channel.epoll.LinuxSocket fd, boolean active) {
        super(fd, active);
        config = new EpollServerSocketChannelConfig(this);
    }
    ...
    protected boolean isCompatible(EventLoop loop) {
        return loop instanceof io.netty.channel.epoll.EpollEventLoop;
    }
    ...
    protected Channel newChildChannel(int fd, byte[] address, int offset, int len) throws Exception {
        return new EpollSocketChannel(this, new io.netty.channel.epoll.LinuxSocket(fd), address(address, offset, len));
    }
    ...
    void setTcpMd5Sig(Map<InetAddress, byte[]> keys) throws IOException {
        tcpMd5SigAddresses = io.netty.channel.epoll.TcpMd5Util.newTcpMd5Sigs(this, tcpMd5SigAddresses, keys);
    }
}
