...
final class EpollEventLoop extends SingleThreadEventLoop {
    private static final InternalLogger logger = InternalLoggerFactory.getInstance(io.netty.channel.epoll.EpollEventLoop.class);
    private static final AtomicIntegerFieldUpdater<io.netty.channel.epoll.EpollEventLoop> WAKEN_UP_UPDATER =
            AtomicIntegerFieldUpdater.newUpdater(io.netty.channel.epoll.EpollEventLoop.class, "wakenUp");
    ...
    private final IntObjectMap<io.netty.channel.epoll.AbstractEpollChannel> channels = new IntObjectHashMap<io.netty.channel.epoll.AbstractEpollChannel>(4096);
    ...
    private final Callable<Integer> pendingTasksCallable = new Callable<Integer>() {
        ...
        public Integer call() throws Exception {
            return io.netty.channel.epoll.EpollEventLoop.super.pendingTasks();
        }
    };
    ...
    EpollEventLoop(EventLoopGroup parent, Executor executor, int maxEvents,
        ...
        if (maxEvents == 0) {
            allowGrowing = true;
            events = new io.netty.channel.epoll.EpollEventArray(4096);
        } else {
            allowGrowing = false;
            events = new io.netty.channel.epoll.EpollEventArray(maxEvents);
        }
     ...
     */
    void add(io.netty.channel.epoll.AbstractEpollChannel ch) throws IOException {
        assert inEventLoop();
     ...
     */
    void modify(io.netty.channel.epoll.AbstractEpollChannel ch) throws IOException {
        assert inEventLoop();
     ...
     */
    void remove(io.netty.channel.epoll.AbstractEpollChannel ch) throws IOException {
        assert inEventLoop();
    ...
    private void closeAll() {
        ...
        Collection<io.netty.channel.epoll.AbstractEpollChannel> array = new ArrayList<io.netty.channel.epoll.AbstractEpollChannel>(channels.size());

        for (io.netty.channel.epoll.AbstractEpollChannel channel: channels.values()) {
            array.add(channel);
        }

        for (io.netty.channel.epoll.AbstractEpollChannel ch: array) {
            ch.unsafe().close(ch.unsafe().voidPromise());
        }
    }

    private void processReady(io.netty.channel.epoll.EpollEventArray events, int ready) {
        for (int i = 0; i < ready; i ++) {
            ...
            if (fd == eventFd.intValue()) {
                ...
                io.netty.channel.epoll.AbstractEpollChannel ch = channels.get(fd);
                if (ch != null) {
