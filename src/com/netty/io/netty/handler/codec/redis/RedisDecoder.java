...
public final class RedisDecoder extends ByteToMessageDecoder {
    ...
    public RedisDecoder(int maxInlineMessageLength, RedisMessagePool messagePool) {
        if (maxInlineMessageLength <= 0 || maxInlineMessageLength > io.netty.handler.codec.redis.RedisConstants.REDIS_MESSAGE_MAX_LENGTH) {
            throw new RedisCodecException("maxInlineMessageLength: " + maxInlineMessageLength +
                                          " (expected: <= " + io.netty.handler.codec.redis.RedisConstants.REDIS_MESSAGE_MAX_LENGTH + ")");
        }
    ...
    private boolean decodeLength(ByteBuf in, List<Object> out) throws Exception {
        ...
        if (length < io.netty.handler.codec.redis.RedisConstants.NULL_VALUE) {
            throw new RedisCodecException("length: " + length + " (expected: >= " + io.netty.handler.codec.redis.RedisConstants.NULL_VALUE + ")");
        }
        switch (type) {
        case ARRAY_HEADER:
            ...
            return true;
        case BULK_STRING:
            if (length > io.netty.handler.codec.redis.RedisConstants.REDIS_MESSAGE_MAX_LENGTH) {
                throw new RedisCodecException("length: " + length + " (expected: <= " +
                                              io.netty.handler.codec.redis.RedisConstants.REDIS_MESSAGE_MAX_LENGTH + ")");
            }
    ...
    private boolean decodeBulkString(ByteBuf in, List<Object> out) throws Exception {
        ...
        case io.netty.handler.codec.redis.RedisConstants.NULL_VALUE: // $-1\r\n
    ...
    private boolean decodeBulkStringEndOfLine(ByteBuf in, List<Object> out) throws Exception {
        if (in.readableBytes() < io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH) {
            return false;
        }
        readEndOfLine(in);
    ...
    private boolean decodeBulkStringContent(ByteBuf in, List<Object> out) throws Exception {
        ...
        if (readableBytes == 0 || remainingBulkLength == 0 && readableBytes < io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH) {
            return false;
        }

        // if this is last frame.
        if (readableBytes >= remainingBulkLength + io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH) {
    ...
    private static void readEndOfLine(final ByteBuf in) {
        ...
        if (io.netty.handler.codec.redis.RedisConstants.EOL_SHORT == delim) {
            return;
        }
        final byte[] bytes = io.netty.handler.codec.redis.RedisCodecUtil.shortToBytes(delim);
    ...
    private static ByteBuf readLine(ByteBuf in) {
        if (!in.isReadable(io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH)) {
            return null;
        }
    ...
    private long parseRedisNumber(ByteBuf byteBuf) {
        ...
        if (readableBytes > io.netty.handler.codec.redis.RedisConstants.POSITIVE_LONG_MAX_LENGTH + extraOneByteForNegative) {
