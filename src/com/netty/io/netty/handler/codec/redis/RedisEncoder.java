...
public class RedisEncoder extends MessageToMessageEncoder<RedisMessage> {
    ...
    private static void writeString(ByteBufAllocator allocator, byte type, String content, List<Object> out) {
        ByteBuf buf = allocator.ioBuffer(io.netty.handler.codec.redis.RedisConstants.TYPE_LENGTH + ByteBufUtil.utf8MaxBytes(content) +
                                         io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH);
        buf.writeByte(type);
        ByteBufUtil.writeUtf8(buf, content);
        buf.writeShort(io.netty.handler.codec.redis.RedisConstants.EOL_SHORT);
        out.add(buf);
    }

    private void writeIntegerMessage(ByteBufAllocator allocator, IntegerRedisMessage msg, List<Object> out) {
        ByteBuf buf = allocator.ioBuffer(io.netty.handler.codec.redis.RedisConstants.TYPE_LENGTH + io.netty.handler.codec.redis.RedisConstants.LONG_MAX_LENGTH +
                                         io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH);
    ...
    private void writeBulkStringHeader(ByteBufAllocator allocator, BulkStringHeaderRedisMessage msg, List<Object> out) {
        final ByteBuf buf = allocator.ioBuffer(io.netty.handler.codec.redis.RedisConstants.TYPE_LENGTH +
                                        (msg.isNull() ? io.netty.handler.codec.redis.RedisConstants.NULL_LENGTH :
                                                        io.netty.handler.codec.redis.RedisConstants.LONG_MAX_LENGTH + io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH));
        buf.writeByte(RedisMessageType.BULK_STRING.value());
        if (msg.isNull()) {
            buf.writeShort(io.netty.handler.codec.redis.RedisConstants.NULL_SHORT);
        } else {
    ...
    private static void writeBulkStringContent(ByteBufAllocator allocator, BulkStringRedisContent msg,
        ...
        if (msg instanceof LastBulkStringRedisContent) {
            out.add(allocator.ioBuffer(io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH).writeShort(io.netty.handler.codec.redis.RedisConstants.EOL_SHORT));
        }
    }
    ...
    private void writeFullBulkStringMessage(ByteBufAllocator allocator, FullBulkStringRedisMessage msg,
                                            List<Object> out) {
        if (msg.isNull()) {
            ByteBuf buf = allocator.ioBuffer(io.netty.handler.codec.redis.RedisConstants.TYPE_LENGTH + io.netty.handler.codec.redis.RedisConstants.NULL_LENGTH +
                                             io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH);
            buf.writeByte(RedisMessageType.BULK_STRING.value());
            buf.writeShort(io.netty.handler.codec.redis.RedisConstants.NULL_SHORT);
            buf.writeShort(io.netty.handler.codec.redis.RedisConstants.EOL_SHORT);
            ...
            ByteBuf headerBuf = allocator.ioBuffer(io.netty.handler.codec.redis.RedisConstants.TYPE_LENGTH + io.netty.handler.codec.redis.RedisConstants.LONG_MAX_LENGTH +
                                                   io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH);
            headerBuf.writeByte(RedisMessageType.BULK_STRING.value());
            headerBuf.writeBytes(numberToBytes(msg.content().readableBytes()));
            headerBuf.writeShort(io.netty.handler.codec.redis.RedisConstants.EOL_SHORT);
            out.add(headerBuf);
            out.add(msg.content().retain());
            out.add(allocator.ioBuffer(io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH).writeShort(io.netty.handler.codec.redis.RedisConstants.EOL_SHORT));
        }
    ...
    private void writeArrayMessage(ByteBufAllocator allocator, ArrayRedisMessage msg, List<Object> out) {
        if (msg.isNull()) {
            writeArrayHeader(allocator, msg.isNull(), io.netty.handler.codec.redis.RedisConstants.NULL_VALUE, out);
        } else {
    ...
    private void writeArrayHeader(ByteBufAllocator allocator, boolean isNull, long length, List<Object> out) {
        if (isNull) {
            final ByteBuf buf = allocator.ioBuffer(io.netty.handler.codec.redis.RedisConstants.TYPE_LENGTH + io.netty.handler.codec.redis.RedisConstants.NULL_LENGTH +
                                                   io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH);
            ...
            final ByteBuf buf = allocator.ioBuffer(io.netty.handler.codec.redis.RedisConstants.TYPE_LENGTH + io.netty.handler.codec.redis.RedisConstants.LONG_MAX_LENGTH +
                                                   io.netty.handler.codec.redis.RedisConstants.EOL_LENGTH);
            buf.writeByte(RedisMessageType.ARRAY_HEADER.value());
