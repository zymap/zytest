...
final class CodecOutputList extends AbstractList<Object> implements RandomAccess {
    ...
    private static final CodecOutputListRecycler NOOP_RECYCLER = new CodecOutputListRecycler() {
        @Override
        public void recycle(io.netty.handler.codec.CodecOutputList object) {
    ...
    private interface CodecOutputListRecycler {
        void recycle(io.netty.handler.codec.CodecOutputList codecOutputList);
    }
    ...
    private static final class CodecOutputLists implements CodecOutputListRecycler {
        ...
        CodecOutputLists(int numElements) {
            elements = new io.netty.handler.codec.CodecOutputList[MathUtil.safeFindNextPositivePowerOfTwo(numElements)];
            for (int i = 0; i < elements.length; ++i) {
                // Size of 16 should be good enough for the majority of all users as an initial capacity.
                elements[i] = new io.netty.handler.codec.CodecOutputList(this, 16);
            }
        ...
        public io.netty.handler.codec.CodecOutputList getOrCreate() {
            if (count == 0) {
                ...
                // low.
                return new io.netty.handler.codec.CodecOutputList(NOOP_RECYCLER, 4);
            }
            --count;

            int idx = (currentIdx - 1) & mask;
            io.netty.handler.codec.CodecOutputList list = elements[idx];
            currentIdx = idx;
            return list;
        }

        @Override
        public void recycle(io.netty.handler.codec.CodecOutputList codecOutputList) {
            int idx = currentIdx;
