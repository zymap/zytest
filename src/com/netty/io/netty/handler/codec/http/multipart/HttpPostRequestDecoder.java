...
public class HttpPostRequestDecoder implements InterfaceHttpPostRequestDecoder {
     ...
     *             errors
     */
    public HttpPostRequestDecoder(io.netty.handler.codec.http.multipart.HttpDataFactory factory, HttpRequest request) {
    ...
    private static String[] splitHeaderContentType(String sb) {
        ...
        int cStart;
        int cEnd;
        aStart = io.netty.handler.codec.http.multipart.HttpPostBodyUtil.findNonWhitespace(sb, 0);
        aEnd =  sb.indexOf(';');
        if (aEnd == -1) {
            return new String[] { sb, "", "" };
        }
        bStart = io.netty.handler.codec.http.multipart.HttpPostBodyUtil.findNonWhitespace(sb, aEnd + 1);
        if (sb.charAt(aEnd - 1) == ' ') {
            aEnd--;
        }
        bEnd =  sb.indexOf(';', bStart);
        if (bEnd == -1) {
            bEnd = io.netty.handler.codec.http.multipart.HttpPostBodyUtil.findEndOfString(sb);
            return new String[] { sb.substring(aStart, aEnd), sb.substring(bStart, bEnd), "" };
        }
        cStart = io.netty.handler.codec.http.multipart.HttpPostBodyUtil.findNonWhitespace(sb, bEnd + 1);
        if (sb.charAt(bEnd - 1) == ' ') {
            bEnd--;
        }
        cEnd = io.netty.handler.codec.http.multipart.HttpPostBodyUtil.findEndOfString(sb);
