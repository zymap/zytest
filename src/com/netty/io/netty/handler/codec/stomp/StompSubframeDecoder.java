...
/**
 * Decodes {@link ByteBuf}s into {@link io.netty.handler.codec.stomp.StompHeadersSubframe}s and
...
public class StompSubframeDecoder extends ReplayingDecoder<State> {
            ...
            switch (state()) {
                case READ_CONTENT:
                    ...
                    if (contentLength >= 0) {
                        ...
                        int nulIndex = indexOf(in, in.readerIndex(), in.writerIndex(), io.netty.handler.codec.stomp.StompConstants.NUL);
    ...
    private static void skipNullCharacter(ByteBuf buffer) {
        ...
        if (b != io.netty.handler.codec.stomp.StompConstants.NUL) {
    ...
    private static void skipControlCharacters(ByteBuf buffer) {
        ...
        for (;;) {
            b = buffer.readByte();
            if (b != io.netty.handler.codec.stomp.StompConstants.CR && b != io.netty.handler.codec.stomp.StompConstants.LF) {
    ...
    private String readLine(ByteBuf buffer, int initialBufferSize) {
        ...
        for (;;) {
            ...
            if (nextByte == io.netty.handler.codec.stomp.StompConstants.CR) {
                //do nothing
            } else if (nextByte == io.netty.handler.codec.stomp.StompConstants.LF) {
                return buf.toString();
            } else {
    ...
    private boolean readHeader(StompHeaders headers, AppendableCharSequence buf, ByteBuf buffer) {
        ...
        for (;;) {
            ...
            if (nextByte == io.netty.handler.codec.stomp.StompConstants.COLON && key == null) {
                key = buf.toString();
                valid = true;
                buf.reset();
            } else if (nextByte == io.netty.handler.codec.stomp.StompConstants.CR) {
                //do nothing
            } else if (nextByte == io.netty.handler.codec.stomp.StompConstants.LF) {
