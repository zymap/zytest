...
final class HpackHuffmanDecoder {
    ...
    private static final io.netty.handler.codec.http2.Http2Exception EOS_DECODED = ThrowableUtil.unknownStackTrace(
            connectionError(COMPRESSION_ERROR, "HPACK - EOS Decoded"), io.netty.handler.codec.http2.HpackHuffmanDecoder.class, "decode(..)");
    private static final io.netty.handler.codec.http2.Http2Exception INVALID_PADDING = ThrowableUtil.unknownStackTrace(
            connectionError(COMPRESSION_ERROR, "HPACK - Invalid Padding"), io.netty.handler.codec.http2.HpackHuffmanDecoder.class, "decode(..)");

    private static final Node ROOT = buildTree(io.netty.handler.codec.http2.HpackUtil.HUFFMAN_CODES, io.netty.handler.codec.http2.HpackUtil.HUFFMAN_CODE_LENGTHS);
     ...
     * @throws io.netty.handler.codec.http2.Http2Exception EOS Decoded
     */
    public AsciiString decode(ByteBuf buf, int length) throws io.netty.handler.codec.http2.Http2Exception {
        processor.reset();
    ...
    private static final class DecoderProcessor implements ByteProcessor {
         ...
         */
        @Override
        public boolean process(byte value) throws io.netty.handler.codec.http2.Http2Exception {
            ...
            do {
                ...
                if (node.isTerminal()) {
                    if (node.symbol == io.netty.handler.codec.http2.HpackUtil.HUFFMAN_EOS) {
                        throw EOS_DECODED;
                    }
                    append(node.symbol);
                    node = ROOT;
