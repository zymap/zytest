...
final class HpackEncoder {
    ...
    private void encodeHeadersEnforceMaxHeaderListSize(int streamId, ByteBuf out, Http2Headers headers,
        ...
        for (Map.Entry<CharSequence, CharSequence> header : headers) {
            ...
            // overflow.
            headerSize += io.netty.handler.codec.http2.HpackHeaderField.sizeOf(name, value);
    ...
    private void encodeHeadersIgnoreMaxHeaderListSize(ByteBuf out, Http2Headers headers,
                                                      SensitivityDetector sensitivityDetector) throws io.netty.handler.codec.http2.Http2Exception {
        for (Map.Entry<CharSequence, CharSequence> header : headers) {
            CharSequence name = header.getKey();
            CharSequence value = header.getValue();
            encodeHeader(out, name, value, sensitivityDetector.isSensitive(name, value),
                         io.netty.handler.codec.http2.HpackHeaderField.sizeOf(name, value));
        }
    }
    ...
    private void encodeHeader(ByteBuf out, CharSequence name, CharSequence value, boolean sensitive, long headerSize) {
        ...
        if (maxHeaderTableSize == 0) {
            int staticTableIndex = io.netty.handler.codec.http2.HpackStaticTable.getIndex(name, value);
            if (staticTableIndex == -1) {
                int nameIndex = io.netty.handler.codec.http2.HpackStaticTable.getIndex(name);
        ...
        if (headerField != null) {
            int index = getIndex(headerField.index) + io.netty.handler.codec.http2.HpackStaticTable.length;
     ...
     */
    public void setMaxHeaderTableSize(ByteBuf out, long maxHeaderTableSize) throws io.netty.handler.codec.http2.Http2Exception {
    ...
    private int getNameIndex(CharSequence name) {
        int index = io.netty.handler.codec.http2.HpackStaticTable.getIndex(name);
        if (index == -1) {
            index = getIndex(name);
            if (index >= 0) {
                index += io.netty.handler.codec.http2.HpackStaticTable.length;
            }
        }
        return index;
    }
     ...
     */
    io.netty.handler.codec.http2.HpackHeaderField getHeaderField(int index) {
     ...
     */
    private static final class HeaderEntry extends io.netty.handler.codec.http2.HpackHeaderField {
