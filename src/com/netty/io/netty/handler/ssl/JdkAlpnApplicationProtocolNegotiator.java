...
public final class JdkAlpnApplicationProtocolNegotiator extends JdkBaseApplicationProtocolNegotiator {
    private static final boolean AVAILABLE = io.netty.handler.ssl.Conscrypt.isAvailable() ||
                                             jdkAlpnSupported() ||
                                             io.netty.handler.ssl.JettyAlpnSslEngine.isAvailable();
    ...
    private static final class AlpnWrapper extends AllocatorAwareSslEngineWrapperFactory {
        ...
        public SSLEngine wrapSslEngine(SSLEngine engine, ByteBufAllocator alloc,
                                       JdkApplicationProtocolNegotiator applicationNegotiator, boolean isServer) {
            if (io.netty.handler.ssl.Conscrypt.isEngineSupported(engine)) {
                return isServer ? io.netty.handler.ssl.ConscryptAlpnSslEngine.newServerEngine(engine, alloc, applicationNegotiator)
                        : io.netty.handler.ssl.ConscryptAlpnSslEngine.newClientEngine(engine, alloc, applicationNegotiator);
            }
            if (jdkAlpnSupported()) {
                return new io.netty.handler.ssl.Java9SslEngine(engine, applicationNegotiator, isServer);
            }
            if (io.netty.handler.ssl.JettyAlpnSslEngine.isAvailable()) {
                return isServer ? io.netty.handler.ssl.JettyAlpnSslEngine.newServerEngine(engine, applicationNegotiator)
                        : io.netty.handler.ssl.JettyAlpnSslEngine.newClientEngine(engine, applicationNegotiator);
            }
    ...
    static boolean jdkAlpnSupported() {
        return PlatformDependent.javaVersion() >= 9 && io.netty.handler.ssl.Java9SslUtils.supportsAlpn();
    }
}
