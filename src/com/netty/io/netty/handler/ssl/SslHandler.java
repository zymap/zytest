 ...
 * exception is when you close the {@link Channel} - {@link io.netty.handler.ssl.SslHandler}
 ...
 * {@link io.netty.handler.ssl.SslHandler} with a new {@link SSLEngine} as explained in the
 * following section.
 *
 * <h3>Restarting the session</h3>
 * <p>
 * To restart the SSL session, you must remove the existing closed
 * {@link io.netty.handler.ssl.SslHandler} from the {@link ChannelPipeline}, insert a new
 * {@link io.netty.handler.ssl.SslHandler} with a new {@link SSLEngine} into the pipeline,
 ...
 * <ol>
 * <li>create a new {@link io.netty.handler.ssl.SslHandler} instance with {@code startTls} flag set
 *     to {@code true},</li>
 * <li>insert the {@link io.netty.handler.ssl.SslHandler} to the {@link ChannelPipeline}, and</li>
 * <li>write a StartTLS response.</li>
 * </ol>
 * Please note that you must insert {@link io.netty.handler.ssl.SslHandler} <em>before</em> sending
 * the StartTLS response.  Otherwise the client can send begin SSL handshake
 * before {@link io.netty.handler.ssl.SslHandler} is inserted to the {@link ChannelPipeline}, causing
 * data corruption.
 * <p>
...
public class SslHandler extends ByteToMessageDecoder implements ChannelOutboundHandler {
    ...
    private static final InternalLogger logger =
            InternalLoggerFactory.getInstance(io.netty.handler.ssl.SslHandler.class);
    ...
    private static final SSLException SSLENGINE_CLOSED = ThrowableUtil.unknownStackTrace(
            new SSLException("SSLEngine closed already"), io.netty.handler.ssl.SslHandler.class, "wrap(...)");
    private static final SSLException HANDSHAKE_TIMED_OUT = ThrowableUtil.unknownStackTrace(
            new SSLException("handshake timed out"), io.netty.handler.ssl.SslHandler.class, "handshake(...)");
    private static final ClosedChannelException CHANNEL_CLOSED = ThrowableUtil.unknownStackTrace(
            new ClosedChannelException(), io.netty.handler.ssl.SslHandler.class, "channelInactive(...)");
    ...
    private enum SslEngineType {
        TCNATIVE(true, COMPOSITE_CUMULATOR) {
            @Override
            SSLEngineResult unwrap(io.netty.handler.ssl.SslHandler handler, ByteBuf in, int readerIndex, int len, ByteBuf out)
                    throws SSLException {
            ...
            @Override
            int getPacketBufferSize(io.netty.handler.ssl.SslHandler handler) {
                return ((ReferenceCountedOpenSslEngine) handler.engine).maxEncryptedPacketLength0();
            }

            @Override
            int calculateWrapBufferCapacity(io.netty.handler.ssl.SslHandler handler, int pendingBytes, int numComponents) {
                return ((ReferenceCountedOpenSslEngine) handler.engine).calculateMaxLengthForWrap(pendingBytes,
                                                                                                  numComponents);
            }

            @Override
            int calculatePendingData(io.netty.handler.ssl.SslHandler handler, int guess) {
        ...
        abstract SSLEngineResult unwrap(io.netty.handler.ssl.SslHandler handler, ByteBuf in, int readerIndex, int len, ByteBuf out)
                throws SSLException;

        abstract int calculateWrapBufferCapacity(io.netty.handler.ssl.SslHandler handler, int pendingBytes, int numComponents);

        abstract int calculatePendingData(io.netty.handler.ssl.SslHandler handler, int guess);
    ...
    private final class SslHandlerCoalescingBufferQueue extends AbstractCoalescingBufferQueue {
        ...
        protected ByteBuf compose(ByteBufAllocator alloc, ByteBuf cumulation, ByteBuf next) {
            final int wrapDataSize = io.netty.handler.ssl.SslHandler.this.wrapDataSize;
