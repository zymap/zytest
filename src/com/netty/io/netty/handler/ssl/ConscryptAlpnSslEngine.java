...
/**
 * A {@link io.netty.handler.ssl.JdkSslEngine} that uses the Conscrypt provider or SSL with ALPN.
 */
abstract class ConscryptAlpnSslEngine extends io.netty.handler.ssl.JdkSslEngine {
    ...
    static io.netty.handler.ssl.ConscryptAlpnSslEngine newClientEngine(SSLEngine engine, ByteBufAllocator alloc,
                                                                       JdkApplicationProtocolNegotiator applicationNegotiator) {
        return new ClientEngine(engine, alloc, applicationNegotiator);
    }

    static io.netty.handler.ssl.ConscryptAlpnSslEngine newServerEngine(SSLEngine engine, ByteBufAllocator alloc,
                                                                       JdkApplicationProtocolNegotiator applicationNegotiator) {
    ...
    private ConscryptAlpnSslEngine(SSLEngine engine, ByteBufAllocator alloc, List<String> protocols) {
        ...
        if (USE_BUFFER_ALLOCATOR) {
            io.netty.handler.ssl.Conscrypt.setBufferAllocator(engine, new BufferAllocatorAdapter(alloc));
        }

        // Set the list of supported ALPN protocols on the engine.
        io.netty.handler.ssl.Conscrypt.setApplicationProtocols(engine, protocols.toArray(new String[protocols.size()]));
    }
    ...
    final int calculateOutNetBufSize(int plaintextBytes, int numBuffers) {
        ...
        long maxOverhead = (long) io.netty.handler.ssl.Conscrypt.maxSealOverhead(getWrappedEngine()) * numBuffers;
    ...
    final SSLEngineResult unwrap(ByteBuffer[] srcs, ByteBuffer[] dests) throws SSLException {
        return io.netty.handler.ssl.Conscrypt.unwrap(getWrappedEngine(), srcs, dests);
    }

    private static final class ClientEngine extends io.netty.handler.ssl.ConscryptAlpnSslEngine {
        ...
        ClientEngine(SSLEngine engine, ByteBufAllocator alloc,
            ...
            io.netty.handler.ssl.Conscrypt.setHandshakeListener(engine, new HandshakeListener() {
                @Override
        ...
        private void selectProtocol() throws SSLException {
            String protocol = io.netty.handler.ssl.Conscrypt.getApplicationProtocol(getWrappedEngine());
            try {
    ...
    private static final class ServerEngine extends io.netty.handler.ssl.ConscryptAlpnSslEngine {
