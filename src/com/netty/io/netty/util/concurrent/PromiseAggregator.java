 ...
 *
 * {@link GenericFutureListener} implementation which consolidates multiple {@link io.netty.util.concurrent.Future}s
 * into one, by listening to individual {@link io.netty.util.concurrent.Future}s and producing an aggregated result
 * (success/failure) when all {@link io.netty.util.concurrent.Future}s have completed.
 *
 * @param <V> the type of value returned by the {@link io.netty.util.concurrent.Future}
 * @param <F> the type of {@link io.netty.util.concurrent.Future}
 */
@Deprecated
public class PromiseAggregator<V, F extends Future<V>> implements GenericFutureListener<F> {
    ...
    /**
     * See {@link io.netty.util.concurrent.PromiseAggregator#PromiseAggregator(Promise, boolean)}.
     ...
     */
    @SafeVarargs
    public final io.netty.util.concurrent.PromiseAggregator<V, F> add(Promise<V>... promises) {
        if (promises == null) {
