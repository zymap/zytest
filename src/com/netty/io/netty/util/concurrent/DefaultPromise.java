...
public class DefaultPromise<V> extends AbstractFuture<V> implements Promise<V> {
    private static final InternalLogger logger = InternalLoggerFactory.getInstance(io.netty.util.concurrent.DefaultPromise.class);
    private static final InternalLogger rejectedExecutionLogger =
            InternalLoggerFactory.getInstance(io.netty.util.concurrent.DefaultPromise.class.getName() + ".rejectedExecution");
    private static final int MAX_LISTENER_STACK_DEPTH = Math.min(8,
            SystemPropertyUtil.getInt("io.netty.defaultPromise.maxListenerStackDepth", 8));
    @SuppressWarnings("rawtypes")
    private static final AtomicReferenceFieldUpdater<io.netty.util.concurrent.DefaultPromise, Object> RESULT_UPDATER =
            AtomicReferenceFieldUpdater.newUpdater(io.netty.util.concurrent.DefaultPromise.class, Object.class, "result");
    private static final Object SUCCESS = new Object();
    private static final Object UNCANCELLABLE = new Object();
    private static final CauseHolder CANCELLATION_CAUSE_HOLDER = new CauseHolder(ThrowableUtil.unknownStackTrace(
            new CancellationException(), io.netty.util.concurrent.DefaultPromise.class, "cancel(...)"));
     ...
     * cannot share code because the listener(s) cannot be cached for an instance of {@link io.netty.util.concurrent.DefaultPromise} since the
    ...
    private void notifyListenersNow() {
        ...
        for (;;) {
            if (listeners instanceof io.netty.util.concurrent.DefaultFutureListeners) {
                notifyListeners0((io.netty.util.concurrent.DefaultFutureListeners) listeners);
            } else {
    ...
    private void notifyListeners0(io.netty.util.concurrent.DefaultFutureListeners listeners) {
    ...
    private void addListener0(GenericFutureListener<? extends Future<? super V>> listener) {
        if (listeners == null) {
            listeners = listener;
        } else if (listeners instanceof io.netty.util.concurrent.DefaultFutureListeners) {
            ((io.netty.util.concurrent.DefaultFutureListeners) listeners).add(listener);
        } else {
            listeners = new io.netty.util.concurrent.DefaultFutureListeners((GenericFutureListener<?>) listeners, listener);
        }
    }

    private void removeListener0(GenericFutureListener<? extends Future<? super V>> listener) {
        if (listeners instanceof io.netty.util.concurrent.DefaultFutureListeners) {
            ((io.netty.util.concurrent.DefaultFutureListeners) listeners).remove(listener);
    ...
    private synchronized Object progressiveListeners() {
        ...
        if (listeners instanceof io.netty.util.concurrent.DefaultFutureListeners) {
            ...
            io.netty.util.concurrent.DefaultFutureListeners dfl = (io.netty.util.concurrent.DefaultFutureListeners) listeners;
