...
public final class AppendableCharSequence implements CharSequence, Appendable {
    ...
    @Override
    public io.netty.util.internal.AppendableCharSequence subSequence(int start, int end) {
        return new io.netty.util.internal.AppendableCharSequence(Arrays.copyOfRange(chars, start, end));
    }

    @Override
    public io.netty.util.internal.AppendableCharSequence append(char c) {
        if (pos == chars.length) {
            char[] old = chars;
    ...
    @Override
    public io.netty.util.internal.AppendableCharSequence append(CharSequence csq) {
        return append(csq, 0, csq.length());
    }

    @Override
    public io.netty.util.internal.AppendableCharSequence append(CharSequence csq, int start, int end) {
        if (csq.length() < end) {
            throw new IndexOutOfBoundsException();
        }
        ...
        if (csq instanceof io.netty.util.internal.AppendableCharSequence) {
            // Optimize append operations via array copy
            io.netty.util.internal.AppendableCharSequence seq = (io.netty.util.internal.AppendableCharSequence) csq;
            char[] src = seq.chars;
