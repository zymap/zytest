 ...
 */
public final class InternalThreadLocalMap extends io.netty.util.internal.UnpaddedInternalThreadLocalMap {

    private static final InternalLogger logger = InternalLoggerFactory.getInstance(io.netty.util.internal.InternalThreadLocalMap.class);
    ...
    static {
        STRING_BUILDER_INITIAL_SIZE =
                io.netty.util.internal.SystemPropertyUtil.getInt("io.netty.threadLocalMap.stringBuilder.initialSize", 1024);
    ...
    private static io.netty.util.internal.InternalThreadLocalMap fastGet(FastThreadLocalThread thread) {
        io.netty.util.internal.InternalThreadLocalMap threadLocalMap = thread.threadLocalMap();
        if (threadLocalMap == null) {
            thread.setThreadLocalMap(threadLocalMap = new io.netty.util.internal.InternalThreadLocalMap());
        }
        return threadLocalMap;
    }

    private static io.netty.util.internal.InternalThreadLocalMap slowGet() {
        ThreadLocal<io.netty.util.internal.InternalThreadLocalMap> slowThreadLocalMap = io.netty.util.internal.UnpaddedInternalThreadLocalMap.slowThreadLocalMap;
        io.netty.util.internal.InternalThreadLocalMap ret = slowThreadLocalMap.get();
        if (ret == null) {
            ret = new io.netty.util.internal.InternalThreadLocalMap();
