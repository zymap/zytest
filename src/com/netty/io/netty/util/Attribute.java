...
public interface Attribute<T> {
    ...
    /**
     *  Atomically sets to the given value if this {@link io.netty.util.Attribute}'s value is {@code null}.
     ...
     *
     * If you only want to return the old value and clear the {@link io.netty.util.Attribute} while still keep it in the
     * {@link AttributeMap} use {@link #getAndSet(Object)} with a value of {@code null}.
     *
     * <p>
     * Be aware that even if you call this method another thread that has obtained a reference to this {@link io.netty.util.Attribute}
     * via {@link AttributeMap#attr(io.netty.util.AttributeKey)} will still operate on the same instance. That said if now another
     * thread or even the same thread later will call {@link AttributeMap#attr(io.netty.util.AttributeKey)} again, a new
     * {@link io.netty.util.Attribute} instance is created and so is not the same as the previous one that was removed. Because of
     ...
     *
     * If you only want to remove the value and clear the {@link io.netty.util.Attribute} while still keep it in
