...
class PooledHeapByteBuf extends io.netty.buffer.PooledByteBuf<byte[]> {

    private static final Recycler<io.netty.buffer.PooledHeapByteBuf> RECYCLER = new Recycler<io.netty.buffer.PooledHeapByteBuf>() {
        @Override
        protected io.netty.buffer.PooledHeapByteBuf newObject(Handle<io.netty.buffer.PooledHeapByteBuf> handle) {
            return new io.netty.buffer.PooledHeapByteBuf(handle, 0);
        }
    };

    static io.netty.buffer.PooledHeapByteBuf newInstance(int maxCapacity) {
        io.netty.buffer.PooledHeapByteBuf buf = RECYCLER.get();
        buf.reuse(maxCapacity);
        return buf;
    }

    PooledHeapByteBuf(Recycler.Handle<? extends io.netty.buffer.PooledHeapByteBuf> recyclerHandle, int maxCapacity) {
