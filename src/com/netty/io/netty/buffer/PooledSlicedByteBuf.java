...
final class PooledSlicedByteBuf extends io.netty.buffer.AbstractPooledDerivedByteBuf {

    private static final Recycler<io.netty.buffer.PooledSlicedByteBuf> RECYCLER = new Recycler<io.netty.buffer.PooledSlicedByteBuf>() {
        @Override
        protected io.netty.buffer.PooledSlicedByteBuf newObject(Handle<io.netty.buffer.PooledSlicedByteBuf> handle) {
            return new io.netty.buffer.PooledSlicedByteBuf(handle);
        }
    };

    static io.netty.buffer.PooledSlicedByteBuf newInstance(AbstractByteBuf unwrapped, ByteBuf wrapped,
                                                           int index, int length) {
        checkSliceOutOfBounds(index, length, unwrapped);
        return newInstance0(unwrapped, wrapped, index, length);
    }

    private static io.netty.buffer.PooledSlicedByteBuf newInstance0(AbstractByteBuf unwrapped, ByteBuf wrapped,
                                                                    int adjustment, int length) {
        final io.netty.buffer.PooledSlicedByteBuf slice = RECYCLER.get();
    ...
    private PooledSlicedByteBuf(Handle<io.netty.buffer.PooledSlicedByteBuf> handle) {
        super(handle);
    }
    ...
    public ByteBuf retainedSlice(int index, int length) {
        checkIndex0(index, length);
        return io.netty.buffer.PooledSlicedByteBuf.newInstance0(unwrap(), this, idx(index), length);
    }
