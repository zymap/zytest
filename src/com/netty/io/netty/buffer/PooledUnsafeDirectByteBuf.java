...
final class PooledUnsafeDirectByteBuf extends io.netty.buffer.PooledByteBuf<ByteBuffer> {
    private static final Recycler<io.netty.buffer.PooledUnsafeDirectByteBuf> RECYCLER = new Recycler<io.netty.buffer.PooledUnsafeDirectByteBuf>() {
        @Override
        protected io.netty.buffer.PooledUnsafeDirectByteBuf newObject(Handle<io.netty.buffer.PooledUnsafeDirectByteBuf> handle) {
            return new io.netty.buffer.PooledUnsafeDirectByteBuf(handle, 0);
        }
    };

    static io.netty.buffer.PooledUnsafeDirectByteBuf newInstance(int maxCapacity) {
        io.netty.buffer.PooledUnsafeDirectByteBuf buf = RECYCLER.get();
    ...
    private PooledUnsafeDirectByteBuf(Recycler.Handle<io.netty.buffer.PooledUnsafeDirectByteBuf> recyclerHandle, int maxCapacity) {
        super(recyclerHandle, maxCapacity);
    }

    @Override
    void init(io.netty.buffer.PoolChunk<ByteBuffer> chunk, long handle, int offset, int length, int maxLength,
    ...
    @Override
    void initUnpooled(io.netty.buffer.PoolChunk<ByteBuffer> chunk, int length) {
